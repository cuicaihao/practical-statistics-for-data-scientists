...
loan_data["outcome"] = pd.Categorical(
    loan_data["outcome"], categories=["paid off", "default"], ordered=True
)
...
loan_tree = DecisionTreeClassifier(
    random_state=1, criterion="entropy", min_impurity_decrease=0.003
)
loan_tree.fit(X, y)
...
for n in n_estimator:
    rf = RandomForestClassifier(
        n_estimators=n, criterion="entropy", max_depth=5, random_state=1, oob_score=True
    )
    rf.fit(X, y)
    oobScores.append(rf.oob_score_)

pd.DataFrame({"n": n_estimator, "oobScore": oobScores}).plot(x="n", y="oobScore")
plt.show()
...
rf_all_entropy = RandomForestClassifier(
    n_estimators=500, random_state=1, criterion="entropy"
)
...
for ntree_limit in range(1, 250):
    ...
    train_penalty = xgb_penalty.predict_proba(train_X, iteration_range=iteration_range)[
        :, 1
    ]
    pred_default = xgb_default.predict_proba(valid_X, iteration_range=iteration_range)[
        :, 1
    ]
    pred_penalty = xgb_penalty.predict_proba(valid_X, iteration_range=iteration_range)[
        :, 1
    ]
    results.append(
        {
