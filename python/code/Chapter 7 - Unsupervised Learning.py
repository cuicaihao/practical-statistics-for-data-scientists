...
ax.set_ylim(-3, 3)
ax.plot(
    *abline(loadings.loc[0, "CVX"] / loadings.loc[0, "XOM"], 0, ax), "--", color="C1"
)
ax.plot(
    *abline(loadings.loc[1, "CVX"] / loadings.loc[1, "XOM"], 0, ax), "--", color="C1"
)
...
for n_components in range(1, 9):
    for covariance_type in covariance_types:
        mclust = GaussianMixture(
            n_components=n_components, warm_start=True, covariance_type=covariance_type
        )
        mclust.fit(df)
        results.append(
            {
...
centers = pd.DataFrame(
    scaler.inverse_transform(kmeans.cluster_centers_), columns=columns
)
centers["size"] = [counts[i] for i in range(4)]
print(centers)
