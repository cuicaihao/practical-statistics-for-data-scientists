...
probabilities = pd.DataFrame(
    naive_model.predict_proba(new_loan), columns=naive_model.classes_
)
print(
...
pred = pd.DataFrame(
    loan_lda.predict_proba(loan3000[predictors]), columns=loan_lda.classes_
)
print(pred.head())
...
outcome = "outcome"
X = pd.get_dummies(
    loan_data[predictors], prefix="", prefix_sep="", drop_first=True, dtype=int
)
...
formula = (
    "outcome ~ bs(payment_inc_ratio, df=8) + purpose_ + "
...
def partialResidualPlot(model, df, outcome, feature, fig, ax):
    ...
    ax.scatter(results.feature, results.residual, marker=".", s=72.0 / fig.dpi)
...
print(
    "percentage of loans in default (SMOTE resampled): ",
...
print(
    "percentage of loans predicted to default (SMOTE): ",
...
print(
    "percentage of loans in default (ADASYN resampled): ",
    100 * np.mean(y_resampled == "default"),
)
...
full_model.fit(X_resampled, y_resampled)
print(
    "percentage of loans predicted to default (ADASYN): ",
    print(100 * np.mean(full_model.predict(X) == "default")),
)
